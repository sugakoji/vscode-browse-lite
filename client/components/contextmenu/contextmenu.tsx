...
class ContextMenu extends React.Component<IContextMenuProps, IContextMenu> {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: IContextMenuProps) {
    if (
  ...
  async manageMenuItemsStatus() {
    ...
    // Delete
    const deleteItem = this.state.menuItems.find(
      x => x.itemType === ContextMenuItemsType.Delete,
    )
    const deleteItemIndex = this.state.menuItems.findIndex(
      x => x.itemType === ContextMenuItemsType.Delete,
    )
    ...
    // Paste & Go
    const pasteGo = this.state.menuItems.find(
      x => x.itemType === ContextMenuItemsType.PasteAndGo,
    )
    const pasteGoIndex = this.state.menuItems.findIndex(
      x => x.itemType === ContextMenuItemsType.PasteAndGo,
    )
    if (
